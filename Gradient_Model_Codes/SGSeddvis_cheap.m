...
function [ugug_SGS,vgvg_SGS,ugvg_SGS,ugUg_SGS,UgUg_SGS,vgVg_SGS,VgVg_SGS,ugVg_SGS,vgUg_SGS,UgVg_SGS,Ug_avg,Vg_avg,h_ave]=SGSeddvis_cheap(eta,f,gp,dx,dy,kfilt,eps,crops)
...
for k=1:size(h,3)
 ...
 ugvg_ave(:,:,k)=centmean_cheap(squeeze(Mask(:,:,k).*ug(:,:,k).*vg(:,:,k)).*A,kfilt)./A_ave;
 ugUg_ave(:,:,k)=centmean_cheap(squeeze(Mask(:,:,k).*ug(:,:,k).*Ug(:,:,k)).*A,kfilt)./A_ave;
 UgUg_ave(:,:,k)=centmean_cheap(squeeze(Mask(:,:,k).*Ug(:,:,k).*Ug(:,:,k)).*A,kfilt)./A_ave;
 vgVg_ave(:,:,k)=centmean_cheap(squeeze(Mask(:,:,k).*vg(:,:,k).*Vg(:,:,k)).*A,kfilt)./A_ave;
 VgVg_ave(:,:,k)=centmean_cheap(squeeze(Mask(:,:,k).*Vg(:,:,k).*Vg(:,:,k)).*A,kfilt)./A_ave;
