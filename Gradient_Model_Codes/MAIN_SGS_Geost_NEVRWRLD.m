...
openstr = 'ocean_geometry_p125.nc'; %% MOM6 OUTPUT
...
openstr = 'snapshots.nc_80y_p0625_Nev_rghnss'; %% MOM6 OUTPUT
fi = netcdf.open(openstr, 'nowrite');
...
nfx = 17;  %% FILTER WIDTH NF = nfx-1 and NF= DELTA_F/DELTA (SEE KHANI AND DAWSON, JAMES 2022)
nfy = 17;
nfxy= nfx;
xsize_f = xsize;
ysize_f = ysize;
...
sea_surf_height = ncread(openstr,'e',[1,1,1,3],[xsize,ysize,1,34]); %% 396
...
for i= 1:tsize
        if (i>=TS)
            ...
            [ugh_sgs,vgh_sgs,h_fxy,vg_fxy,ug_fxy,vg,ugh_fxy,ug_fxyh_fxy,vgh_fxy,vg_fxyh_fxy,ug_sgs,vg_sgs,h_sgs]=SGSflux_cheap_NV(eee,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%% crops 1 is the best option 
            [ugh_sgs0,vgh_sgs0,h_fxy0,vg_fxy0,ug_fxy0,vg0,ugh_fxy0,ug_fxyh_fxy0,vgh_fxy0,vg_fxyh_fxy0]=SGSflux_cheap_NV(eee,CORCOEF,gprime,DX,DY,nfxy,eps,0);
            [ugh_sgs2,vgh_sgs2,h_fxy2,vg_fxy2,ug_fxy2,vg2,ugh_fxy2,ug_fxyh_fxy2,vgh_fxy2,vg_fxyh_fxy2]=SGSflux_cheap_NV(eee,CORCOEF,gprime,DX,DY,nfxy,eps,0);
            
            [ugug_sgs,vgvg_sgs,ugvg_sgs,ugUg_sgs,UgUg_sgs,vgVg_sgs,VgVg_sgs,ugVg_sgs,vgUg_sgs,UgVg_sgs,Ug_fxy,Vg_fxy,h_fxy_mom]=SGSeddvis_cheap_NV(eee,CORCOEF,gprime,DX,DY,nfxy,eps,1);
...
for i= 1:tsize
        if (i>=TS)
            ...
            eee3 = ncread(openstr,'e',[1,1,1,i],[xsize,ysize,zsize+1,1]);
            eee2(eee2<-1.e33) = NaN;

            eee2(:,:,1:zsize) = eee2(:,:,1:zsize) - eeta(:,:,1:zsize);
            eee3(eee3<-1.e33) = NaN;
            eee2(eee2<-1.e33) = NaN;
            
            [ugh_sgs_TR,vgh_sgs_TR,h_fxy_TR,ugh_fxy_TR,ug_fxyh_fxy_TR,vgh_fxy_TR,vg_fxyh_fxy_TR,ug_sgs_TR,vg_sgs_TR,h_sgs_TR]=SGSflux_cheap_NV(eee2,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%% crops 1 is the best option 
            [ugh_sgs,vgh_sgs,h_fxy,ugh_fxy,ug_fxyh_fxy,vgh_fxy,vg_fxyh_fxy,ug_sgs,vg_sgs,h_sgs]=SGSflux_cheap_NV(eee3,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%% crops 1 is the best option  
            [ugug_sgs_TR,vgvg_sgs_TR,ugvg_sgs_TR,ugUg_sgs_TR,UgUg_sgs_TR,vgVg_sgs_TR,VgVg_sgs_TR,ugVg_sgs_TR,vgUg_sgs_TR,UgVg_sgs_TR,Ug_fxy_TR,Vg_fxy_TR,h_fxy_momTR]=SGSeddvis_cheap_NV(eee2,CORCOEF,gprime,DX,DY,nfxy,eps,1); 
...
vgh_sgsta_TR = vgh_sgsta_TR / sum_av2;
...
[ugh_sgsta_ST1,vgh_sgsta_ST1,hta_fxy_ST1,ugh_fxyta_ST1,ug_fxyh_fxyta_ST1,vgh_fxyta_ST1,vg_fxyh_fxyta_ST1,ug_sgsta_ST1,vg_sgsta_ST1,h_sgsta_ST1]=SGSflux_cheap_NV(eeta,CORCOEF,gprime,DX,DY,nfxy,eps,1);
 eeta(isnan(eeta))=0;
EKE_sgsta_ST = (ug_sgsta_ST1 .* ug_sgsta_ST1 + vg_sgsta_ST1 .* vg_sgsta_ST1);
...
%%% IF MASKING IS REQUIRED FOR NEVERWORLD: TURN ON THE FOLLOWINGS
...
mkdir NWR
cd NWR
mkdir Filt_17 %%% CHANGE THIS IF YOU USE DIFFERENT FILTER WIDTH nfx, nfy 
cd Filt_17
