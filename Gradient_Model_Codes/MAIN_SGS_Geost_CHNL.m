...
%%%%%%%% PARAMETERIZATIONS INCLUDE 1. GRADIENT MODEL
%%%%%%%% 2. GM MODEL
%%%%%%%% 3. THICKNESS HEIGHT MODEL
%%%%%%%% CONFIGURATIONS USING MOM6 NETCDF DATA. PLEASE SEE THE GITHAB PAGES:
%%%%%%%% https://github.com/sinakhani/Gradient-parameterization-in-OGCM
%%%%%%%% https://github.com/sinakhani/Neverworld
%%%%%%%% https://github.com/sinakhani/MOM6
%%%%%%%% PUBLICATIONS:
%%%%%%%% (https://agupubs.onlinelibrary.wiley.com/doi/abs/10.1029/2019MS001721)
%%%%%%%% (KHANI ET AL., JAMES 2019)
%%%%%%%% (KHANI AND DAWSON, JAMES 2022)
%%%%%%%% THIS SCRIPT IS WRITTEN BY SINA KHANI (sina.khani@austin.utexas.edu) 
%%%%%%%% & (skhani@princeton.edu).
...
openstr = 'ocean_geometry_p125_Channel_rghnss.nc'; %% MOM6 OUTPUT
...
openstr = 'snapshots.nc_80y_p125_KT2_Channel_rghnss'; %% MOM6 OUTPUT
fi = netcdf.open(openstr, 'nowrite');
%%% get the varibles to plot, there are many to choose from
%%% this for loop will tell you where each variable is stored,
[numdims, numvars, numglobalatts, unlimdimID] = netcdf.inq(fi);
...
tsize = 730;%% TIME OUTPUT
nyr = 10;   
avyr = 0.25; 
TS = 729; %%365;  %% TIME AVERAGING OVER TS<t<tsize

tsize = 730; %% TIME OUTPUT
nyr = 10;   %% DO NOT CHAGE THIS 
avyr = 0.25;   %% FOR 1/8 in movmean use 0.25 and for 1/16 and 1/4 (3650) use 0.25/4
TS = 365; %% TIME AVERAGING OVER TS<t<tsize
...
e_fxy_TR = zeros(xsize_f,ysize_f,zsize,'single');
...
h_fxyta = zeros(xsize_f,ysize_f,zsize,'single');
h_fxy_momta = zeros(xsize_f,ysize_f,zsize,'single');
vg_fxyta = zeros(xsize_f,ysize_f,zsize,'single');
...
nl_grad_vgh_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_ugh_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_ugug_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_vgvg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_ugvg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_ugUg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_UgUg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_vgVg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_VgVg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_ugVg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_vgUg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
nl_grad_UgVg_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
GM_vgh_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
GM_ugh_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
KH_vgh_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
KH_ugh_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
RHS_NLX_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
RHS_GMX_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
RHS_NLY_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
RHS_GMY_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
RHS_ACX_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
RHS_ACY_sgsta = zeros(xsize_f,ysize_f,zsize,'single');
...
ugvg_sgsta  = zeros(xsize_f,ysize_f,zsize,'single');
vgVg_sgsta  = zeros(xsize_f,ysize_f,zsize,'single');
VgVg_sgsta  = zeros(xsize_f,ysize_f,zsize,'single');
ugUg_sgsta  = zeros(xsize_f,ysize_f,zsize,'single');
UgUg_sgsta  = zeros(xsize_f,ysize_f,zsize,'single');
ugVg_sgsta  = zeros(xsize_f,ysize_f,zsize,'single');
vgUg_sgsta  = zeros(xsize_f,ysize_f,zsize,'single');
UgVg_sgsta  = zeros(xsize_f,ysize_f,zsize,'single');
...
denomh_sgsT2_TRtaG = zeros(xsize_f,ysize_f,zsize,'single');
denomh_sgsMOT2_TRtaG   = zeros(xsize_f,ysize_f,zsize,'single');     
denomh_sgsT2ta_STG = zeros(xsize_f,ysize_f,zsize,'single');
denomh_sgsMOT2ta_STG = zeros(xsize_f,ysize_f,zsize,'single');

AR6_fxy3D = zeros(xsize_f,ysize_f,zsize,'single');
...
sea_surf_height = ncread(openstr,'e',[1,1,1,396],[xsize,ysize,1,1]); 
bathymetry      = ncread(openstr,'e',[1,1,7,396],[xsize,ysize,1,1]); 
...
for i= 1:tsize
        if (i>=TS)
            ...
            [ugh_sgs,vgh_sgs,h_fxy,vg_fxy,ug_fxy,vg,ugh_fxy,ug_fxyh_fxy,vgh_fxy,vg_fxyh_fxy,ug_sgs,vg_sgs,h_sgs]=SGSflux_cheap(eee,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%% crops 1 is the best option 
            [ugh_sgs0,vgh_sgs0,h_fxy0,ugh_fxy0,ug_fxyh_fxy0,vgh_fxy0,vg_fxyh_fxy0]=SGSflux_filtfirst_cheap(eee,CORCOEF,gprime,DX,DY,nfxy,eps,0);
            [ugh_sgs2,vgh_sgs2,h_fxy2,ugh_fxy2,ug_fxyh_fxy2,vgh_fxy2,vg_fxyh_fxy2]=SGSflux_filtfirst_cheap(eee,CORCOEF,gprime,DX,DY,nfxy,eps,0);
            
            [ugug_sgs,vgvg_sgs,ugvg_sgs,ugUg_sgs,UgUg_sgs,vgVg_sgs,VgVg_sgs,ugVg_sgs,vgUg_sgs,UgVg_sgs,Ug_fxy,Vg_fxy,h_fxy_mom]=SGSeddvis_cheap(eee,CORCOEF,gprime,DX,DY,nfxy,eps,1);
            ...
            vg_fxy(isnan(vg_fxy))=0;
            ug_fxy(isnan(ug_fxy))=0;
            vg(isnan(vg))=0;
            
                        
            vgh_sgsreg = vgh_sgs .*MASK_reg;
            h_fxyreg   = h_fxy .* MASK_reg;

            ugug_sgs(isnan(ugug_sgs))=0;
            vgvg_sgs(isnan(vgvg_sgs))=0;
            ugvg_sgs(isnan(ugvg_sgs))=0;
            ...
            VgVg_sgs(isnan(VgVg_sgs))=0;
            ugVg_sgs(isnan(ugVg_sgs))=0;
            vgUg_sgs(isnan(vgUg_sgs))=0;
            UgVg_sgs(isnan(UgVg_sgs))=0;
            h_fxy_mom(isnan(h_fxy_mom))=0;

            
            EKE_sgs = (ug_sgs .*ug_sgs  + vg_sgs .* vg_sgs) ;
            ...
            for l=1:xsize_f
                for m=1:ysize_f
                    vghb_sgs(l,m,:) = vgh_sgs(l,m,:) - (h_fxy(l,m,:) * (sum_vghsgs(l,m)/sum_hfxy(l,m)));%% EDDY FLUXES WHEN BAROTROPIC PART IS REMOVED
            ...
            [dudx,dudy]=grad(ug_fxy,DX_fxy,DY_fxy);
            [dVdx,dVdy]=grad(Vg_fxy,DX_fxy,DY_fxy);
            [dUdx,dUdy]=grad(Ug_fxy,DX_fxy,DY_fxy);
            ...
            %%% GRADIENT MODEL, GM-MODEL, THICKNESS HEIGHT MODEL

            nl_grad_vgh_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dvdx(:,:,:) .* dhdx(:,:,:)) + (dvdy(:,:,:) .* dhdy(:,:,:)) );
            nl_grad_ugh_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dudx(:,:,:) .* dhdx(:,:,:)) + (dudy(:,:,:) .* dhdy(:,:,:)) );
            
            nl_grad_ugug_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dudx(:,:,:) .* dudx(:,:,:)) + (dudy(:,:,:) .* dudy(:,:,:)) );
            nl_grad_vgvg_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dvdx(:,:,:) .* dvdx(:,:,:)) + (dvdy(:,:,:) .* dvdy(:,:,:)) );
            nl_grad_ugvg_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dudx(:,:,:) .* dvdx(:,:,:)) + (dudy(:,:,:) .* dvdy(:,:,:)) );
            ...
            nl_grad_VgVg_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dVdx(:,:,:) .* dVdx(:,:,:)) + (dVdy(:,:,:) .* dVdy(:,:,:)) );
            nl_grad_ugVg_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dudx(:,:,:) .* dVdx(:,:,:)) + (dudy(:,:,:) .* dVdy(:,:,:)) );
            nl_grad_vgUg_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dvdx(:,:,:) .* dUdx(:,:,:)) + (dvdy(:,:,:) .* dUdy(:,:,:)) );
            nl_grad_UgVg_sgs(:,:,:) = AR6_fxy3D(:,:,:).*( (dUdx(:,:,:) .* dVdx(:,:,:)) + (dUdy(:,:,:) .* dVdy(:,:,:)) );
            

            [dNLXdx,dNLXdy] =grad(nl_grad_ugh_sgs,DX_fxy,DY_fxy);
            [dNLYdx,dNLYdy] =grad(nl_grad_vgh_sgs,DX_fxy,DY_fxy);
            RHS_NLX_sgs(:,:,:) = (dhdx(:,:,:) .* nl_grad_ugh_sgs(:,:,:));  
            RHS_NLY_sgs(:,:,:) = (dhdy(:,:,:) .* nl_grad_vgh_sgs(:,:,:)); 
            RHS_ACX_sgs(:,:,:) = (dhdx(:,:,:) .* ugh_sgs(:,:,:));
            RHS_ACY_sgs(:,:,:) = (dhdy(:,:,:) .* vgh_sgs(:,:,:)); 
           
            
            GM_vgh_sgs(:,:,:) = K_GM *( dedy(:,:,:) );
            GM_ugh_sgs(:,:,:) = K_GM *( dedx(:,:,:) );
            
            KH_vgh_sgs(:,:,:) = K_GM *( dhdy(:,:,:) );
            KH_ugh_sgs(:,:,:) = K_GM *( dhdx(:,:,:) );

            [dGMXdx,dGMXdy] =grad(dhdx,DX_fxy,DY_fxy);
            [dGMYdx,dGMYdy] =grad(dhdy,DX_fxy,DY_fxy);
            RHS_GMX_sgs(:,:,:) = K_GM* (dhdx(:,:,:) .* dhdx(:,:,:)); 
            RHS_GMY_sgs(:,:,:) = K_GM* (dhdy(:,:,:) .* dhdy(:,:,:)); 
 ...
 %%% TIME AVERAGING               
            ...
            sum_av = sum_av +1;
            h_fxyta(:,:,:) = h_fxyta(:,:,:) + h_fxy(:,:,:);
            h_fxy_momta(:,:,:) = h_fxy_momta(:,:,:) + h_fxy_mom(:,:,:);
            vg_fxyta(:,:,:) = vg_fxyta(:,:,:) + vg_fxy(:,:,:);
            vgta(:,:,:) = vgta(:,:,:) + vg(:,:,:);
            eeta(:,:,:) = eeta(:,:,:) + ee(:,:,:);
            ...
            nl_grad_vgh_sgsta(:,:,:) = nl_grad_vgh_sgsta(:,:,:)   + nl_grad_vgh_sgs(:,:,:); 
            nl_grad_ugh_sgsta(:,:,:) = nl_grad_ugh_sgsta(:,:,:)   + nl_grad_ugh_sgs(:,:,:); 
            nl_grad_ugug_sgsta(:,:,:) = nl_grad_ugug_sgsta(:,:,:)   + nl_grad_ugug_sgs(:,:,:);
            nl_grad_vgvg_sgsta(:,:,:) = nl_grad_vgvg_sgsta(:,:,:)   + nl_grad_vgvg_sgs(:,:,:);
            nl_grad_ugvg_sgsta(:,:,:) = nl_grad_ugvg_sgsta(:,:,:)   + nl_grad_ugvg_sgs(:,:,:);
            nl_grad_ugUg_sgsta(:,:,:) = nl_grad_ugUg_sgsta(:,:,:)   + nl_grad_ugUg_sgs(:,:,:);
            nl_grad_UgUg_sgsta(:,:,:) = nl_grad_UgUg_sgsta(:,:,:)   + nl_grad_UgUg_sgs(:,:,:);
            nl_grad_vgVg_sgsta(:,:,:) = nl_grad_vgVg_sgsta(:,:,:)   + nl_grad_vgVg_sgs(:,:,:);
            nl_grad_VgVg_sgsta(:,:,:) = nl_grad_VgVg_sgsta(:,:,:)   + nl_grad_VgVg_sgs(:,:,:);
            nl_grad_ugVg_sgsta(:,:,:) = nl_grad_ugVg_sgsta(:,:,:)   + nl_grad_ugVg_sgs(:,:,:);
            nl_grad_vgUg_sgsta(:,:,:) = nl_grad_vgUg_sgsta(:,:,:)   + nl_grad_vgUg_sgs(:,:,:);
            nl_grad_UgVg_sgsta(:,:,:) = nl_grad_UgVg_sgsta(:,:,:)   + nl_grad_UgVg_sgs(:,:,:);
            GM_vgh_sgsta(:,:,:) = GM_vgh_sgsta(:,:,:)   + GM_vgh_sgs(:,:,:); 
            GM_ugh_sgsta(:,:,:) = GM_ugh_sgsta(:,:,:)   + GM_ugh_sgs(:,:,:);            
            KH_vgh_sgsta(:,:,:) = KH_vgh_sgsta(:,:,:)   + KH_vgh_sgs(:,:,:);
            KH_ugh_sgsta(:,:,:) = KH_ugh_sgsta(:,:,:)   + KH_ugh_sgs(:,:,:);
            RHS_NLX_sgsta(:,:,:) = RHS_NLX_sgsta(:,:,:)   + RHS_NLX_sgs(:,:,:); 
            RHS_GMX_sgsta(:,:,:) = RHS_GMX_sgsta(:,:,:)   + RHS_GMX_sgs(:,:,:); 
            RHS_NLY_sgsta(:,:,:) = RHS_NLY_sgsta(:,:,:)   + RHS_NLY_sgs(:,:,:); 
            RHS_GMY_sgsta(:,:,:) = RHS_GMY_sgsta(:,:,:)   + RHS_GMY_sgs(:,:,:);
            RHS_ACX_sgsta(:,:,:) = RHS_ACX_sgsta(:,:,:)   + RHS_ACX_sgs(:,:,:);
            RHS_ACY_sgsta(:,:,:) = RHS_ACY_sgsta(:,:,:)   + RHS_ACY_sgs(:,:,:); 
            ...
            ugvg_sgsta(:,:,:) = ugvg_sgsta(:,:,:) + ugvg_sgs(:,:,:);
            vgVg_sgsta(:,:,:) = vgVg_sgsta(:,:,:) + vgVg_sgs(:,:,:);
            VgVg_sgsta(:,:,:) = VgVg_sgsta(:,:,:) + VgVg_sgs(:,:,:);
            ugUg_sgsta(:,:,:) = ugUg_sgsta(:,:,:) + ugUg_sgs(:,:,:);
            UgUg_sgsta(:,:,:) = UgUg_sgsta(:,:,:) + UgUg_sgs(:,:,:);
            ugVg_sgsta(:,:,:) = ugVg_sgsta(:,:,:) + ugVg_sgs(:,:,:);
            vgUg_sgsta(:,:,:) = vgUg_sgsta(:,:,:) + vgUg_sgs(:,:,:);
            UgVg_sgsta(:,:,:) = UgVg_sgsta(:,:,:) + UgVg_sgs(:,:,:);
...
h_fxyta = h_fxyta / sum_av;
h_fxy_momta=h_fxy_momta / sum_av;
eeta = eeta / sum_av;

mean_h_fxyta = nanmean(h_fxyta(:));
h_fxyta(isnan(h_fxyta))=0;

[ugh_sgsta_ST,vgh_sgsta_ST,hta_fxy_ST,vgta_fxy_ST,ugta_fxy_ST,vgta_ST,ugh_fxyta_ST,ug_fxyh_fxyta_ST,vgh_fxyta_ST,vg_fxyh_fxyta_ST,ug_sgsta_ST,vg_sgsta_ST,h_sgsta_ST]=SGSflux_cheap(eeta,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%%%% crops 1 is the best so far
[ugug_sgsta_ST,vgvg_sgsta_ST,ugvg_sgsta_ST]=SGSeddvis_cheap(eeta,CORCOEF,gprime,DX,DY,nfxy,eps,1);
...
ugvg_sgsta_ST(isnan(ugvg_sgsta_ST))=0;
...
nl_grad_vgh_sgsta = nl_grad_vgh_sgsta /sum_av;
nl_grad_ugh_sgsta = nl_grad_ugh_sgsta /sum_av;
nl_grad_ugug_sgsta = nl_grad_ugug_sgsta /sum_av;
nl_grad_vgvg_sgsta = nl_grad_vgvg_sgsta /sum_av;
nl_grad_ugvg_sgsta = nl_grad_ugvg_sgsta /sum_av;
nl_grad_ugUg_sgsta = nl_grad_ugUg_sgsta /sum_av;
nl_grad_UgUg_sgsta = nl_grad_UgUg_sgsta /sum_av;
nl_grad_vgVg_sgsta = nl_grad_vgVg_sgsta /sum_av;
nl_grad_VgVg_sgsta = nl_grad_VgVg_sgsta /sum_av;
nl_grad_ugVg_sgsta = nl_grad_ugVg_sgsta /sum_av;
nl_grad_vgUg_sgsta = nl_grad_vgUg_sgsta /sum_av;
nl_grad_UgVg_sgsta = nl_grad_UgVg_sgsta /sum_av;
GM_vgh_sgsta = GM_vgh_sgsta / sum_av;
GM_ugh_sgsta = GM_ugh_sgsta / sum_av;
KH_vgh_sgsta = KH_vgh_sgsta / sum_av;
KH_ugh_sgsta = KH_ugh_sgsta / sum_av;
RHS_NLX_sgsta = RHS_NLX_sgsta / sum_av;
RHS_GMX_sgsta = RHS_GMX_sgsta / sum_av;
RHS_NLY_sgsta = RHS_NLY_sgsta / sum_av;
RHS_GMY_sgsta = RHS_GMY_sgsta / sum_av;
RHS_ACX_sgsta = RHS_ACX_sgsta / sum_av;
RHS_ACY_sgsta = RHS_ACY_sgsta / sum_av;
...
ugvg_sgsta = ugvg_sgsta / sum_av;
vgVg_sgsta = vgVg_sgsta / sum_av;
VgVg_sgsta = VgVg_sgsta / sum_av;
ugUg_sgsta = ugUg_sgsta / sum_av;
UgUg_sgsta = UgUg_sgsta / sum_av;
ugVg_sgsta = ugVg_sgsta / sum_av;
vgUg_sgsta = vgUg_sgsta / sum_av;
UgVg_sgsta = UgVg_sgsta / sum_av;
...
EKE_sgsta_ST = (ug_sgsta_ST .* ug_sgsta_ST + vg_sgsta_ST .* vg_sgsta_ST);
...
%%% DECOMPOSE INTO TRANSIENT & STANDING PARTS
...
ugvg_sgsta_TR = zeros(xsize_f,ysize_f,zsize,'single');
ugUg_sgsta_TR = zeros(xsize_f,ysize_f,zsize,'single');
UgUg_sgsta_TR = zeros(xsize_f,ysize_f,zsize,'single');
vgVg_sgsta_TR = zeros(xsize_f,ysize_f,zsize,'single');
VgVg_sgsta_TR = zeros(xsize_f,ysize_f,zsize,'single');
ugVg_sgsta_TR = zeros(xsize_f,ysize_f,zsize,'single');
vgUg_sgsta_TR = zeros(xsize_f,ysize_f,zsize,'single');
UgVg_sgsta_TR = zeros(xsize_f,ysize_f,zsize,'single');


nomh_sgsT2_TRta  = zeros(xsize_f,ysize_f,zsize,'single');
denomh_sgsT2_TRta = zeros(xsize_f,ysize_f,zsize,'single');
denomh_sgsMOT2_TRta = zeros(xsize_f,ysize_f,zsize,'single');
...
for i= 1:tsize
        if (i>=TS)
            e2 = ncread(openstr,'e',[1,1,1,i],[xsize,ysize,zsize,1]);
            ...
            for m=1:zsize
                AR6_fxy = centmean_cheap(AR_6(:,:,m),nfxy);
                e_fxy_TR(:,:,m) = centmean(e2(:,:,m) .* AR_6(:,:,m),nfxy) ./ AR6_fxy;
            end
            
            
            [ugh_sgs_TR,vgh_sgs_TR,h_fxy_TR,vg_fxy_TR,ug_fxy_TR,vg_TR,ugh_fxy_TR,ug_fxyh_fxy_TR,vgh_fxy_TR,vg_fxyh_fxy_TR,ug_sgs_TR,vg_sgs_TR,h_sgs_TR]=SGSflux_cheap(eee2,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%% crops 1 is the best option 
            [ugug_sgs_TR,vgvg_sgs_TR,ugvg_sgs_TR,ugUg_sgs_TR,UgUg_sgs_TR,vgVg_sgs_TR,VgVg_sgs_TR,ugVg_sgs_TR,vgUg_sgs_TR,UgVg_sgs_TR,Ug_fxy_TR,Vg_fxy_TR,h_fxy_momTR]=SGSeddvis_cheap(eee2,CORCOEF,gprime,DX,DY,nfxy,eps,1); 
            
            ugh_sgs_TR(isnan(ugh_sgs_TR))=0;
            vgh_sgs_TR(isnan(vgh_sgs_TR))=0;             
            h_fxy_TR(isnan(h_fxy_TR))=0;
            e_fxy_TR(isnan(e_fxy_TR))=0;
            ...
            ugvg_sgs_TR(isnan(ugvg_sgs_TR))=0;

            Ug_fxy_TR(isnan(Ug_fxy_TR))=0;
            Vg_fxy_TR(isnan(Vg_fxy_TR))=0;
            ugUg_sgs_TR(isnan(ugUg_sgs_TR))=0;
            UgUg_sgs_TR(isnan(UgUg_sgs_TR))=0;
            vgVg_sgs_TR(isnan(vgVg_sgs_TR))=0;
            VgVg_sgs_TR(isnan(VgVg_sgs_TR))=0;
            ugVg_sgs_TR(isnan(ugVg_sgs_TR))=0;
            vgUg_sgs_TR(isnan(vgUg_sgs_TR))=0;
            UgVg_sgs_TR(isnan(UgVg_sgs_TR))=0;
            h_fxy_momTR(isnan(h_fxy_momTR))=0;
            ...
            ugvg_sgs_TR = ugvg_sgs_TR - ugvg_sgsta_ST;
            ug_sgs_TR  = ug_sgs_TR  - ug_sgsta_ST;
            vg_sgs_TR  = vg_sgs_TR  - vg_sgsta_ST;
            
            EKE_sgs_TR = (ug_sgs_TR .* ug_sgs_TR + vg_sgs_TR .* vg_sgs_TR);
            VEL_sgs_TR = real(EKE_sgs_TR.^0.5);
            
            [dhdx,dhdy]=grad(h_fxy_TR,DX_fxy,DY_fxy);
            [dedx,dedy]=grad(e_fxy_TR,DX_fxy,DY_fxy);
            ...
            vgh_sgs_fluxTR(:,:,1) =  vgh_sgs_TR(:,:,6);
            ugh_sgs_fluxTR(:,:,1) =  ugh_sgs_TR(:,:,6);

            for k=2:zsize
                vgh_sgs_fluxTR(:,:,k) =  ( vgh_sgs_TR(:,:,zsize+1-k) ) + vgh_sgs_fluxTR(:,:,k-1);
                ugh_sgs_fluxTR(:,:,k) =  ( ugh_sgs_TR(:,:,zsize+1-k) ) + ugh_sgs_fluxTR(:,:,k-1);    
            end
            vgh_sgs_fluxTR = flip(vgh_sgs_fluxTR,3);
            ugh_sgs_fluxTR = flip(ugh_sgs_fluxTR,3);

                                    
            nomh_sgsT2_TR = ( (vgh_sgs_TR .* dhdy) + (ugh_sgs_TR .* dhdx));
            denomh_sgsT2_TR = (DHDX2 + DHDY2);
            
            
            
            nomh_sgsMOT2_TR = ( (vgh_sgs_TR .* dhdyMO) + (ugh_sgs_TR .* dhdx)); 
            denomh_sgsMOT2_TR = (DHDX2 + DHDY2MO);
            ...
            ugvg_sgsta_TR(:,:,:) = ugvg_sgsta_TR(:,:,:) + ugvg_sgs_TR(:,:,:);
            ugUg_sgsta_TR(:,:,:) = ugUg_sgsta_TR(:,:,:) + ugUg_sgs_TR(:,:,:);
            UgUg_sgsta_TR(:,:,:) = UgUg_sgsta_TR(:,:,:) + UgUg_sgs_TR(:,:,:);
            vgVg_sgsta_TR(:,:,:) = vgVg_sgsta_TR(:,:,:) + vgVg_sgs_TR(:,:,:);
            VgVg_sgsta_TR(:,:,:) = VgVg_sgsta_TR(:,:,:) + VgVg_sgs_TR(:,:,:);
            ugVg_sgsta_TR(:,:,:) = ugVg_sgsta_TR(:,:,:) + ugVg_sgs_TR(:,:,:);
            vgUg_sgsta_TR(:,:,:) = vgUg_sgsta_TR(:,:,:) + vgUg_sgs_TR(:,:,:);
            UgVg_sgsta_TR(:,:,:) = UgVg_sgsta_TR(:,:,:) + UgVg_sgs_TR(:,:,:);
            nomh_sgsT2_TRta(:,:,:) = nomh_sgsT2_TRta(:,:,:) + nomh_sgsT2_TR(:,:,:);
            denomh_sgsT2_TRta(:,:,:) = denomh_sgsT2_TRta(:,:,:) + denomh_sgsT2_TR(:,:,:);
            denomh_sgsMOT2_TRta(:,:,:) = denomh_sgsMOT2_TRta(:,:,:) + denomh_sgsMOT2_TR(:,:,:);
...
%%vgh_sgsta_TR = vgh_sgsta_TR / sum_av2; %% DEFINE BELOW
%%ugh_sgsta_TR = ugh_sgsta_TR / sum_av2;
ugug_sgsta_TR = ugug_sgsta_TR / sum_av2;
vgvg_sgsta_TR = vgvg_sgsta_TR / sum_av2;
ugvg_sgsta_TR = ugvg_sgsta_TR / sum_av2;
ugUg_sgsta_TR = ugUg_sgsta_TR / sum_av2;
UgUg_sgsta_TR = UgUg_sgsta_TR / sum_av2;
vgVg_sgsta_TR = vgVg_sgsta_TR / sum_av2;
VgVg_sgsta_TR = VgVg_sgsta_TR / sum_av2;
ugVg_sgsta_TR = ugVg_sgsta_TR / sum_av2;
vgUg_sgsta_TR = vgUg_sgsta_TR / sum_av2;
UgVg_sgsta_TR = UgVg_sgsta_TR / sum_av2;


nomh_sgsT2_TRta = nomh_sgsT2_TRta / sum_av2;
denomh_sgsT2_TRta = denomh_sgsT2_TRta /sum_av2; 
denomh_sgsMOT2_TRta =denomh_sgsMOT2_TRta / sum_av2;
...
vgh_sgsta_ST = vgh_sgsta - vgh_sgsta_TR;
ugh_sgsta_ST = ugh_sgsta - ugh_sgsta_TR;



ugug_sgsta_ST = ugug_sgsta - ugug_sgsta_TR;
vgvg_sgsta_ST = vgvg_sgsta - vgvg_sgsta_TR;
ugvg_sgsta_ST = ugvg_sgsta - ugvg_sgsta_TR;
ugUg_sgsta_ST = ugUg_sgsta - ugUg_sgsta_TR;
UgUg_sgsta_ST = UgUg_sgsta - UgUg_sgsta_TR;
vgVg_sgsta_ST = vgVg_sgsta - vgVg_sgsta_TR;
VgVg_sgsta_ST = VgVg_sgsta - VgVg_sgsta_TR;
ugVg_sgsta_ST = ugVg_sgsta - ugVg_sgsta_TR;
vgUg_sgsta_ST = vgUg_sgsta - vgUg_sgsta_TR;
UgVg_sgsta_ST = UgVg_sgsta - UgVg_sgsta_TR;
...
ugvg_sgsta_TR = ugvg_sgsta - ugvg_sgsta_ST; 

denomh_sgsT2ta_ST = (dhdxta .* dhdxta) + (dhdyta .* dhdyta);
denomh_sgsMOT2ta_ST = (dhdxta .* dhdxta) + (dhdyMOta .* dhdyMOta);
...
denom_MixL_sgsT2ta_ST =  (VEL_sgsta_ST .* VEL_sgsta_ST) .* (denomh_sgsT2ta_ST);  
denom_MixL_sgsMOT2ta_ST =  (VEL_sgsta_ST .* VEL_sgsta_ST) .* (denomh_sgsMOT2ta_ST);  
...
%%% ZONAL INTEGRATION (YOU CAN ALSO DO THIS OFF-LINE USING TIME-AVERAGING
%%% OUTPUTS.
...
ugvg_sgsta_za = zeros(ysize_f,zsize,'single');
vgvg_sgstaST_za = zeros(ysize_f,zsize,'single');
ugug_sgstaST_za = zeros(ysize_f,zsize,'single');
ugvg_sgstaST_za = zeros(ysize_f,zsize,'single');
vgvg_sgstaTR_za = zeros(ysize_f,zsize,'single');
ugug_sgstaTR_za = zeros(ysize_f,zsize,'single');
ugvg_sgstaTR_za = zeros(ysize_f,zsize,'single');
nl_grad_vgh_sgsta_za = zeros(ysize_f,zsize,'single');
nl_grad_ugh_sgsta_za = zeros(ysize_f,zsize,'single');
nl_grad_ugug_sgsta_za = zeros(ysize_f,zsize,'single');
nl_grad_vgvg_sgsta_za = zeros(ysize_f,zsize,'single');
nl_grad_ugvg_sgsta_za = zeros(ysize_f,zsize,'single');
GM_vgh_sgsta_za = zeros(ysize_f,zsize,'single');
GM_ugh_sgsta_za = zeros(ysize_f,zsize,'single');
RHS_NLX_sgsta_za = zeros(ysize_f,zsize,'single');
RHS_GMX_sgsta_za = zeros(ysize_f,zsize,'single');
RHS_NLY_sgsta_za = zeros(ysize_f,zsize,'single');
RHS_GMY_sgsta_za = zeros(ysize_f,zsize,'single');
RHS_ACX_sgsta_za = zeros(ysize_f,zsize,'single');
RHS_ACY_sgsta_za = zeros(ysize_f,zsize,'single');
...
nl_grad_vgh_sgsta = nl_grad_vgh_sgsta .* MASK;
nl_grad_ugh_sgsta = nl_grad_ugh_sgsta .* MASK;
GM_vgh_sgsta = GM_vgh_sgsta .* MASK;
GM_ugh_sgsta = GM_ugh_sgsta .* MASK;
RHS_NLX_sgsta = RHS_NLX_sgsta .* MASK;
RHS_GMX_sgsta = RHS_GMX_sgsta .* MASK;
RHS_NLY_sgsta = RHS_NLY_sgsta .* MASK;
RHS_GMY_sgsta = RHS_GMY_sgsta .* MASK;
RHS_ACX_sgsta = RHS_ACX_sgsta .* MASK;
RHS_ACY_sgsta = RHS_ACY_sgsta .* MASK;
...
denomh_sgsT2_TRta = denomh_sgsT2_TRta .* MASK;
denomh_sgsT2ta_ST = denomh_sgsT2ta_ST .* MASK;
denomh_sgsMOT2_TRta = denomh_sgsMOT2_TRta .* MASK;
denomh_sgsMOT2ta_ST = denomh_sgsMOT2ta_ST .* MASK;
...
ugvg_sgsta = ugvg_sgsta .* MASK;
nl_grad_ugug_sgsta = nl_grad_ugug_sgsta .* MASK;
nl_grad_vgvg_sgsta = nl_grad_vgvg_sgsta .* MASK;
nl_grad_ugvg_sgsta = nl_grad_ugvg_sgsta .* MASK;
vgvg_sgsta_ST = vgvg_sgsta_ST .* MASK;
ugug_sgsta_ST = ugug_sgsta_ST .* MASK;
ugvg_sgsta_ST = ugvg_sgsta_ST .* MASK;
vgvg_sgsta_TR = vgvg_sgsta_TR .* MASK;
ugug_sgsta_TR = ugug_sgsta_TR .* MASK;
ugvg_sgsta_TR = ugvg_sgsta_TR .* MASK;
    ...
    for k=1:zsize
        for i=1:xsize_f
            if (MASK(i,j,k) ==1)
               ...
               nl_grad_vgh_sgsta_za(j,k) = nl_grad_vgh_sgsta_za(j,k) + (nl_grad_vgh_sgsta(i,j,k)  .*  DX_fxy(i,j));
               nl_grad_ugh_sgsta_za(j,k) = nl_grad_ugh_sgsta_za(j,k) + (nl_grad_ugh_sgsta(i,j,k));
               GM_vgh_sgsta_za(j,k) = GM_vgh_sgsta_za(j,k) + (GM_vgh_sgsta(i,j,k)  .*  DX_fxy(i,j));
               GM_ugh_sgsta_za(j,k) = GM_ugh_sgsta_za(j,k) + (GM_ugh_sgsta(i,j,k));
               RHS_NLX_sgsta_za(j,k) = RHS_NLX_sgsta_za(j,k) + (RHS_NLX_sgsta(i,j,k));
               RHS_GMX_sgsta_za(j,k) = RHS_GMX_sgsta_za(j,k) + (RHS_GMX_sgsta(i,j,k));
               RHS_NLY_sgsta_za(j,k) = RHS_NLY_sgsta_za(j,k) + (RHS_NLY_sgsta(i,j,k) .*  DX_fxy(i,j));
               RHS_GMY_sgsta_za(j,k) = RHS_GMY_sgsta_za(j,k) + (RHS_GMY_sgsta(i,j,k) .*  DX_fxy(i,j));
               RHS_ACX_sgsta_za(j,k) = RHS_ACX_sgsta_za(j,k) + (RHS_ACX_sgsta(i,j,k));
               RHS_ACY_sgsta_za(j,k) = RHS_ACY_sgsta_za(j,k) + (RHS_ACY_sgsta(i,j,k) .*  DX_fxy(i,j));  
               ...
               ugvg_sgsta_za(j,k) = ugvg_sgsta_za(j,k) + (ugvg_sgsta(i,j,k).*hta_fxy(i,j,k));
               ...
               ugvg_sgstaST_za(j,k) = ugvg_sgstaST_za(j,k) + (ugvg_sgsta_ST(i,j,k).*hta_fxy(i,j,k));
               ...
               ugvg_sgstaTR_za(j,k) = ugvg_sgstaTR_za(j,k) + (ugvg_sgsta_TR(i,j,k).*hta_fxy(i,j,k)); 
               nl_grad_ugug_sgsta_za(j,k) = nl_grad_ugug_sgsta_za(j,k) + (nl_grad_ugug_sgsta(i,j,k).*hta_fxy(i,j,k));
               nl_grad_vgvg_sgsta_za(j,k) = nl_grad_vgvg_sgsta_za(j,k) + (nl_grad_vgvg_sgsta(i,j,k).*hta_fxy(i,j,k));
               nl_grad_ugvg_sgsta_za(j,k) = nl_grad_ugvg_sgsta_za(j,k) + (nl_grad_ugvg_sgsta(i,j,k).*hta_fxy(i,j,k));
...
DY_za     = DY_za / sum_ef;
DX_za     = DX_za / sum_ef;
ugug_sgsta_za = (ugug_sgsta_za ./ hta_fxy_za) / sum_ef;
vgvg_sgsta_za = (vgvg_sgsta_za ./ hta_fxy_za) / sum_ef;
ugvg_sgsta_za = (ugvg_sgsta_za ./ hta_fxy_za) / sum_ef;
ugug_sgstaST_za = (ugug_sgstaST_za ./ hta_fxy_za) / sum_ef;
vgvg_sgstaST_za = (vgvg_sgstaST_za ./ hta_fxy_za) / sum_ef;
ugvg_sgstaST_za = (ugvg_sgstaST_za ./ hta_fxy_za) / sum_ef;
ugug_sgstaTR_za = (ugug_sgstaTR_za ./ hta_fxy_za) / sum_ef;
vgvg_sgstaTR_za = (vgvg_sgstaTR_za ./ hta_fxy_za) / sum_ef;
ugvg_sgstaTR_za = (ugvg_sgstaTR_za ./ hta_fxy_za) / sum_ef;
nl_grad_ugug_sgsta_za = (nl_grad_ugug_sgsta_za ./ hta_fxy_za) / sum_ef;
nl_grad_vgvg_sgsta_za = (nl_grad_vgvg_sgsta_za ./ hta_fxy_za) / sum_ef;
nl_grad_ugvg_sgsta_za = (nl_grad_ugvg_sgsta_za ./ hta_fxy_za) / sum_ef;
...
nl_grad_ugh_sgsta_za = nl_grad_ugh_sgsta_za / sum_ef;
GM_ugh_sgsta_za = GM_ugh_sgsta_za / sum_ef;
...
ENG_nl_grad_sgstaza = (1/2) * ( (nl_grad_ugug_sgsta_za .* nl_grad_ugug_sgsta_za) + (nl_grad_vgvg_sgsta_za .* nl_grad_vgvg_sgsta_za) ).^(0.5);
...
%%%%%%%%%%%%%%%%%% MAX OR MIB BAROCLINIC TRANSPORT 
...
for k=1:zsize             
    for i=1:xsize_f
        for j=1:ysize_f
            if (denomh_sgsT2ta(i,j,k) > 2.5e-6)       
                MASK_GA(i,j,k) = 0;
                sum_MASK = sum_MASK +1;
            end
        end
...
for i=1:xsize_f
    for j=1:ysize_f
        ...
        denomh_sgsT2_TRtaG(i,j,:) = denomh_sgsT2_TRta(i,j,:) *  AR_fxy(i,j);
        denomh_sgsMOT2_TRtaG(i,j,:) = denomh_sgsMOT2_TRta(i,j,:) *  AR_fxy(i,j);        
        denomh_sgsT2ta_STG(i,j,:) = denomh_sgsT2ta_ST(i,j,:) *  AR_fxy(i,j);
        denomh_sgsMOT2ta_STG(i,j,:) = denomh_sgsMOT2ta_ST(i,j,:) *  AR_fxy(i,j);
...
nom_sgsta_G = nom_sgsta_G .* MASK_GA .* MASK; %% .* (h_fxyta / mean_h_fxyta);
denom_sgsta_G = denom_sgsta_G .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nombS_sgsta_G = nombS_sgsta_G .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nomh_sgsta_G = nomh_sgsta_G .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
denomh_sgsta_G = denomh_sgsta_G .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nomh_sgsT1ta_G = nomh_sgsT1ta_G .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
denomh_sgsT1ta_G = denomh_sgsT1ta_G .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nomh_sgsT2ta_G = nomh_sgsT2ta_G .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nomh_sgsT2ta_TRG = nomh_sgsT2ta_TRG .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nomh_sgsT2ta_STG = nomh_sgsT2ta_STG .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nomh_sgsMOT2ta_G = nomh_sgsMOT2ta_G .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nomh_sgsMOT2ta_TRG = nomh_sgsMOT2ta_TRG .* MASK_GA .* MASK; %%   .* (h_fxyta / mean_h_fxyta);
nomh_sgsMOT2ta_STG = nomh_sgsMOT2ta_STG .* MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denomh_sgsT2ta_G = denomh_sgsT2ta_G .* MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denomh_sgsMOT2ta_G = denomh_sgsMOT2ta_G .* MASK_GA .* MASK; %% .* (h_fxyta / mean_h_fxyta);
denomh_sgsT2ta_STG = denomh_sgsT2ta_STG .* MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denomh_sgsMOT2ta_STG = denomh_sgsMOT2ta_STG .* MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denomh_sgsT2_TRtaG = denomh_sgsT2_TRtaG .* MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denomh_sgsMOT2_TRtaG = denomh_sgsMOT2_TRtaG .* MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
Kappa_edta_G =  Kappa_edta_G .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
Kappa_edMOta_G =  Kappa_edMOta_G .*  MASK_GA .* MASK; %% .* (h_fxyta / mean_h_fxyta);
nomh_sgsT2_TRtaG = nomh_sgsT2_TRtaG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
nomh_sgsMOT2_TRtaG = nomh_sgsMOT2_TRtaG .*  MASK_GA .* MASK; %% .* (h_fxyta / mean_h_fxyta);
nom_MixL_sgsT2ta_G    =  nom_MixL_sgsT2ta_G .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);  
nom_MixL_sgsT2_TRtaG =  nom_MixL_sgsT2_TRtaG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
nom_MixL_sgsT2ta_STG =  nom_MixL_sgsT2ta_STG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
nom_MixL_sgsMOT2ta_G    =  nom_MixL_sgsMOT2ta_G .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);  
nom_MixL_sgsMOT2_TRtaG =  nom_MixL_sgsMOT2_TRtaG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
nom_MixL_sgsMOT2ta_STG =  nom_MixL_sgsMOT2ta_STG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denom_MixL_sgsT2ta_G    =  denom_MixL_sgsT2ta_G .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);  
denom_MixL_sgsT2_TRtaG =  denom_MixL_sgsT2_TRtaG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denom_MixL_sgsT2ta_STG =  denom_MixL_sgsT2ta_STG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denom_MixL_sgsMOT2ta_G    =  denom_MixL_sgsMOT2ta_G .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);  
denom_MixL_sgsMOT2_TRtaG =  denom_MixL_sgsMOT2_TRtaG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
denom_MixL_sgsMOT2ta_STG =  denom_MixL_sgsMOT2ta_STG .*  MASK_GA .* MASK; %%  .* (h_fxyta / mean_h_fxyta);
VEL_sgsta_TRG = VEL_sgsta_TRG.*  MASK_GA .* MASK .* (h_fxyta / mean_h_fxyta);
VEL_sgsta_STG = VEL_sgsta_STG .*  MASK_GA .* MASK  .* (h_fxyta / mean_h_fxyta);
VEL_sgsta_G = VEL_sgsta_G .*  MASK_GA .* MASK  .* (h_fxyta / mean_h_fxyta);
...
denomh_sgsT2_TRtaGA=0;
denomh_sgsMOT2_TRtaGA=0;
denomh_sgsT2ta_STGA=0;
denomh_sgsMOT2ta_STGA=0;
denomh_sgsMOta_GA=0;
Kappa_edta_GA = 0;
Kappa_edMOta_GA = 0;
...
denomh_sgsT2_TRtaGA1=0;
denomh_sgsMOT2_TRtaGA1=0;
denomh_sgsT2ta_STGA1=0;
denomh_sgsMOT2ta_STGA1=0;
...
denomh_sgsT2_TRtaGA2=0;
denomh_sgsMOT2_TRtaGA2=0;
denomh_sgsT2ta_STGA2=0;
denomh_sgsMOT2ta_STGA2=0;
...
denomh_sgsT2ta_STG(isnan(denomh_sgsT2ta_STG)) = 0;
denomh_sgsMOT2ta_STG(isnan(denomh_sgsMOT2ta_STG)) = 0;
denomh_sgsT2_TRtaG(isnan(denomh_sgsT2_TRtaG)) = 0;
denomh_sgsMOT2_TRtaG(isnan(denomh_sgsMOT2_TRtaG)) = 0;
...
for k=1:zsize
    for i=1:(xsize_f)  %%%
        for j=1:(ysize_f) %%
                ...
                denomh_sgsT2ta_STGA = denomh_sgsT2ta_STGA + denomh_sgsT2ta_STG(i,j,k);
                denomh_sgsMOT2ta_STGA = denomh_sgsMOT2ta_STGA + denomh_sgsMOT2ta_STG(i,j,k);
                denomh_sgsT2_TRtaGA = denomh_sgsT2_TRtaGA + denomh_sgsT2_TRtaG(i,j,k);
                denomh_sgsMOT2_TRtaGA = denomh_sgsMOT2_TRtaGA + denomh_sgsMOT2_TRtaG(i,j,k);
                ...
                if(Y_f(j) >= -33.3)    %%% -33.3 For channel 
                ...
                denomh_sgsT2ta_STGA1 = denomh_sgsT2ta_STGA1 + denomh_sgsT2ta_STG(i,j,k);
                denomh_sgsMOT2ta_STGA1 = denomh_sgsMOT2ta_STGA1 + denomh_sgsMOT2ta_STG(i,j,k);
                denomh_sgsT2_TRtaGA1 = denomh_sgsT2_TRtaGA1 + denomh_sgsT2_TRtaG(i,j,k);
                denomh_sgsMOT2_TRtaGA1 = denomh_sgsMOT2_TRtaGA1 + denomh_sgsMOT2_TRtaG(i,j,k);
                ...
                elseif (Y_f(j) < -33.3)  %%% -33.3 For channel 
                ...
                denomh_sgsT2ta_STGA2 = denomh_sgsT2ta_STGA2 + denomh_sgsT2ta_STG(i,j,k);
                denomh_sgsMOT2ta_STGA2 = denomh_sgsMOT2ta_STGA2 + denomh_sgsMOT2ta_STG(i,j,k);
                denomh_sgsT2_TRtaGA2 = denomh_sgsT2_TRtaGA2 + denomh_sgsT2_TRtaG(i,j,k);
                denomh_sgsMOT2_TRtaGA2 = denomh_sgsMOT2_TRtaGA2 + denomh_sgsMOT2_TRtaG(i,j,k);   
...
KHTH_STGhT2 = -nomh_sgsT2ta_STGA / denomh_sgsT2ta_STGA;
...
KHTH_STGhMOT2 = -nomh_sgsMOT2ta_STGA / denomh_sgsMOT2ta_STGA;
...
KHTH_STGhT21 = -nomh_sgsT2ta_STGA1 / denomh_sgsT2ta_STGA1;
...
KHTH_STGhMOT21 = -nomh_sgsMOT2ta_STGA1 / denomh_sgsMOT2ta_STGA1;
...
KHTH_STGhT22 = -nomh_sgsT2ta_STGA2 / denomh_sgsT2ta_STGA2;
...
KHTH_STGhMOT22 = -nomh_sgsMOT2ta_STGA2 / denomh_sgsMOT2ta_STGA2;
...
denomh_sgsT2_TRta_SD1 = denomh_sgsT2_TRtaG - ( (denomh_sgsT2_TRtaGA1 / sum_glob1 )^0.5 * denomh_sgsT2_TRtaG.^0.5 );  
denomh_sgsMOT2_TRta_SD1 = denomh_sgsMOT2_TRtaG - ( (denomh_sgsMOT2_TRtaGA1 / sum_glob1)^0.5 * denomh_sgsMOT2_TRtaG.^0.5 );
denomh_sgsT2ta_ST_SD1 = denomh_sgsT2ta_STG - ( (denomh_sgsT2ta_STGA1 / sum_glob1 )^0.5 * denomh_sgsT2ta_STG.^0.5 );    
denomh_sgsMOT2ta_ST_SD1 = denomh_sgsMOT2ta_STG - ( (denomh_sgsMOT2ta_STGA1 / sum_glob1)^0.5 * denomh_sgsMOT2ta_STG.^0.5 );
...
denom_MixL_sgsT2_TRta_SD1 = denom_MixL_sgsT2_TRtaG - ( (denom_MixL_sgsT2_TRtaGA1 / sum_glob1 )^0.5 * denom_MixL_sgsT2_TRtaG.^0.5 );             
denom_MixL_sgsMOT2_TRta_SD1 = denom_MixL_sgsMOT2_TRtaG - ( (denom_MixL_sgsMOT2_TRtaGA1 / sum_glob1)^0.5 * denom_MixL_sgsMOT2_TRtaG.^0.5 );
denom_MixL_sgsT2ta_ST_SD1 = denom_MixL_sgsT2ta_STG - ( (denom_MixL_sgsT2ta_STGA1 / sum_glob1 )^0.5 * denom_MixL_sgsT2ta_STG.^0.5 );             
denom_MixL_sgsMOT2ta_ST_SD1 = denom_MixL_sgsMOT2ta_STG - ( (denom_MixL_sgsMOT2ta_STGA1 / sum_glob1)^0.5 * denom_MixL_sgsMOT2ta_STG.^0.5 );

nomh_sgsT2ta_SD1 = nomh_sgsT2ta_SD1 .* nomh_sgsT2ta_SD1 .* MASK_GA ;
nomh_sgsT2_TRta_SD1 = nomh_sgsT2_TRta_SD1 .* nomh_sgsT2_TRta_SD1 .* MASK_GA;
nomh_sgsT2ta_ST_SD1 = nomh_sgsT2ta_ST_SD1 .* nomh_sgsT2ta_ST_SD1 .* MASK_GA;
nomh_sgsMOT2ta_SD1 = nomh_sgsMOT2ta_SD1 .* nomh_sgsMOT2ta_SD1 .* MASK_GA;
nomh_sgsMOT2_TRta_SD1 = nomh_sgsMOT2_TRta_SD1 .* nomh_sgsMOT2_TRta_SD1 .* MASK_GA;
nomh_sgsMOT2ta_ST_SD1 = nomh_sgsMOT2ta_ST_SD1 .* nomh_sgsMOT2ta_ST_SD1 .* MASK_GA;
denomh_sgsT2ta_SD1 = denomh_sgsT2ta_SD1 .* denomh_sgsT2ta_SD1 .* MASK_GA;
denomh_sgsMOT2ta_SD1 = denomh_sgsMOT2ta_SD1 .* denomh_sgsMOT2ta_SD1 .* MASK_GA;
denomh_sgsT2_TRta_SD1 = denomh_sgsT2_TRta_SD1 .* denomh_sgsT2_TRta_SD1 .* MASK_GA;
denomh_sgsMOT2_TRta_SD1 = denomh_sgsMOT2_TRta_SD1 .* denomh_sgsMOT2_TRta_SD1 .* MASK_GA;
denomh_sgsT2ta_ST_SD1 = denomh_sgsT2ta_ST_SD1 .* denomh_sgsT2ta_ST_SD1 .* MASK_GA;
denomh_sgsMOT2ta_ST_SD1 = denomh_sgsMOT2ta_ST_SD1 .* denomh_sgsMOT2ta_ST_SD1 .* MASK_GA;

nom_MixL_sgsT2ta_SD1 = nom_MixL_sgsT2ta_SD1 .* nom_MixL_sgsT2ta_SD1 .* MASK_GA ;
nom_MixL_sgsT2_TRta_SD1 = nom_MixL_sgsT2_TRta_SD1 .* nom_MixL_sgsT2_TRta_SD1 .* MASK_GA;
nom_MixL_sgsT2ta_ST_SD1 = nom_MixL_sgsT2ta_ST_SD1 .* nom_MixL_sgsT2ta_ST_SD1 .* MASK_GA;
nom_MixL_sgsMOT2ta_SD1 = nom_MixL_sgsMOT2ta_SD1 .* nom_MixL_sgsMOT2ta_SD1 .* MASK_GA;
nom_MixL_sgsMOT2_TRta_SD1 = nom_MixL_sgsMOT2_TRta_SD1 .* nom_MixL_sgsMOT2_TRta_SD1 .* MASK_GA;
nom_MixL_sgsMOT2ta_ST_SD1 = nom_MixL_sgsMOT2ta_ST_SD1 .* nom_MixL_sgsMOT2ta_ST_SD1 .* MASK_GA;
denom_MixL_sgsT2ta_SD1 = denom_MixL_sgsT2ta_SD1 .* denom_MixL_sgsT2ta_SD1 .* MASK_GA;
denom_MixL_sgsMOT2ta_SD1 = denom_MixL_sgsMOT2ta_SD1 .* denom_MixL_sgsMOT2ta_SD1 .* MASK_GA;
denom_MixL_sgsT2_TRta_SD1 = denom_MixL_sgsT2_TRta_SD1 .* denom_MixL_sgsT2_TRta_SD1 .* MASK_GA;
denom_MixL_sgsMOT2_TRta_SD1 = denom_MixL_sgsMOT2_TRta_SD1 .* denom_MixL_sgsMOT2_TRta_SD1 .* MASK_GA;
denom_MixL_sgsT2ta_ST_SD1 = denom_MixL_sgsT2ta_ST_SD1 .* denom_MixL_sgsT2ta_ST_SD1 .* MASK_GA;
denom_MixL_sgsMOT2ta_ST_SD1 = denom_MixL_sgsMOT2ta_ST_SD1 .* denom_MixL_sgsMOT2ta_ST_SD1 .* MASK_GA;
...
denomh_sgsT2_TRta_SD2 = denomh_sgsT2_TRtaG - ( (denomh_sgsT2_TRtaGA2 / sum_glob2 )^0.5 * denomh_sgsT2_TRtaG.^0.5 );             
denomh_sgsMOT2_TRta_SD2 = denomh_sgsMOT2_TRtaG - ( (denomh_sgsMOT2_TRtaGA2 / sum_glob2)^0.5 * denomh_sgsMOT2_TRtaG.^0.5 );
denomh_sgsT2ta_ST_SD2 = denomh_sgsT2ta_STG - ( (denomh_sgsT2ta_STGA2 / sum_glob2 )^0.5 * denomh_sgsT2ta_STG.^0.5 );             
denomh_sgsMOT2ta_ST_SD2 = denomh_sgsMOT2ta_STG - ( (denomh_sgsMOT2ta_STGA2 / sum_glob2)^0.5 * denomh_sgsMOT2ta_STG.^0.5 );
...
denom_MixL_sgsT2_TRta_SD2 = denom_MixL_sgsT2_TRtaG - ( (denom_MixL_sgsT2_TRtaGA2 / sum_glob2 )^0.5 * denom_MixL_sgsT2_TRtaG.^0.5 );             
denom_MixL_sgsMOT2_TRta_SD2 = denom_MixL_sgsMOT2_TRtaG - ( (denom_MixL_sgsMOT2_TRtaGA2 / sum_glob2)^0.5 * denom_MixL_sgsMOT2_TRtaG.^0.5 );
denom_MixL_sgsT2ta_ST_SD2 = denom_MixL_sgsT2ta_STG - ( (denom_MixL_sgsT2ta_STGA2 / sum_glob2 )^0.5 * denom_MixL_sgsT2ta_STG.^0.5 );             
denom_MixL_sgsMOT2ta_ST_SD2 = denom_MixL_sgsMOT2ta_STG - ( (denom_MixL_sgsMOT2ta_STGA2 / sum_glob2)^0.5 * denom_MixL_sgsMOT2ta_STG.^0.5 );

nomh_sgsT2ta_SD2 = nomh_sgsT2ta_SD2 .* nomh_sgsT2ta_SD2 .* MASK_GA ;
nomh_sgsT2_TRta_SD2 = nomh_sgsT2_TRta_SD2 .* nomh_sgsT2_TRta_SD2 .* MASK_GA;
nomh_sgsT2ta_ST_SD2 = nomh_sgsT2ta_ST_SD2 .* nomh_sgsT2ta_ST_SD2 .* MASK_GA;
nomh_sgsMOT2ta_SD2 = nomh_sgsMOT2ta_SD2 .* nomh_sgsMOT2ta_SD2 .* MASK_GA;
nomh_sgsMOT2_TRta_SD2 = nomh_sgsMOT2_TRta_SD2 .* nomh_sgsMOT2_TRta_SD2 .* MASK_GA;
nomh_sgsMOT2ta_ST_SD2 = nomh_sgsMOT2ta_ST_SD2 .* nomh_sgsMOT2ta_ST_SD2 .* MASK_GA;
denomh_sgsT2ta_SD2 = denomh_sgsT2ta_SD2 .* denomh_sgsT2ta_SD2 .* MASK_GA;
denomh_sgsMOT2ta_SD2 = denomh_sgsMOT2ta_SD2 .* denomh_sgsMOT2ta_SD2 .* MASK_GA;
denomh_sgsT2_TRta_SD2 = denomh_sgsT2_TRta_SD2 .* denomh_sgsT2_TRta_SD2 .* MASK_GA;
denomh_sgsMOT2_TRta_SD2 = denomh_sgsMOT2_TRta_SD2 .* denomh_sgsMOT2_TRta_SD2 .* MASK_GA;
denomh_sgsT2ta_ST_SD2 = denomh_sgsT2ta_ST_SD2 .* denomh_sgsT2ta_ST_SD2 .* MASK_GA;
denomh_sgsMOT2ta_ST_SD2 = denomh_sgsMOT2ta_ST_SD2 .* denomh_sgsMOT2ta_ST_SD2 .* MASK_GA;

nom_MixL_sgsT2ta_SD2 = nom_MixL_sgsT2ta_SD2 .* nom_MixL_sgsT2ta_SD2 .* MASK_GA ;
nom_MixL_sgsT2_TRta_SD2 = nom_MixL_sgsT2_TRta_SD2 .* nom_MixL_sgsT2_TRta_SD2 .* MASK_GA;
nom_MixL_sgsT2ta_ST_SD2 = nom_MixL_sgsT2ta_ST_SD2 .* nom_MixL_sgsT2ta_ST_SD2 .* MASK_GA;
nom_MixL_sgsMOT2ta_SD2 = nom_MixL_sgsMOT2ta_SD2 .* nom_MixL_sgsMOT2ta_SD2 .* MASK_GA;
nom_MixL_sgsMOT2_TRta_SD2 = nom_MixL_sgsMOT2_TRta_SD2 .* nom_MixL_sgsMOT2_TRta_SD2 .* MASK_GA;
nom_MixL_sgsMOT2ta_ST_SD2 = nom_MixL_sgsMOT2ta_ST_SD2 .* nom_MixL_sgsMOT2ta_ST_SD2 .* MASK_GA;
denom_MixL_sgsT2ta_SD2 = denom_MixL_sgsT2ta_SD2 .* denom_MixL_sgsT2ta_SD2 .* MASK_GA;
denom_MixL_sgsMOT2ta_SD2 = denom_MixL_sgsMOT2ta_SD2 .* denom_MixL_sgsMOT2ta_SD2 .* MASK_GA;
denom_MixL_sgsT2_TRta_SD2 = denom_MixL_sgsT2_TRta_SD2 .* denom_MixL_sgsT2_TRta_SD2 .* MASK_GA;
denom_MixL_sgsMOT2_TRta_SD2 = denom_MixL_sgsMOT2_TRta_SD2 .* denom_MixL_sgsMOT2_TRta_SD2 .* MASK_GA;
denom_MixL_sgsT2ta_ST_SD2 = denom_MixL_sgsT2ta_ST_SD2 .* denom_MixL_sgsT2ta_ST_SD2 .* MASK_GA;
denom_MixL_sgsMOT2ta_ST_SD2 = denom_MixL_sgsMOT2ta_ST_SD2 .* denom_MixL_sgsMOT2ta_ST_SD2 .* MASK_GA;
...
denomh_sgsT2_TRta_SD1_GA = 0;      
denomh_sgsMOT2_TRta_SD1_GA = 0;
denomh_sgsT2ta_ST_SD1_GA = 0;      
denomh_sgsMOT2ta_ST_SD1_GA = 0;
...
denom_MixL_sgsT2_TRta_SD1_GA = 0;      
denom_MixL_sgsMOT2_TRta_SD1_GA = 0;
denom_MixL_sgsT2ta_ST_SD1_GA = 0;      
denom_MixL_sgsMOT2ta_ST_SD1_GA = 0;
...
denomh_sgsT2_TRta_SD2_GA = 0;      
denomh_sgsMOT2_TRta_SD2_GA = 0;
denomh_sgsT2ta_ST_SD2_GA = 0;      
denomh_sgsMOT2ta_ST_SD2_GA = 0;
...
denom_MixL_sgsT2_TRta_SD2_GA = 0;      
denom_MixL_sgsMOT2_TRta_SD2_GA = 0;
denom_MixL_sgsT2ta_ST_SD2_GA = 0;      
denom_MixL_sgsMOT2ta_ST_SD2_GA = 0;
    ...
    for i=1:(xsize_f)  %%%
        for j=1:(ysize_f)
             ...
             if(Y_f(j) >= -33.3)    %%% -33.3 For channel 
                 ...
                 denomh_sgsT2_TRta_SD1_GA = denomh_sgsT2_TRta_SD1_GA + denomh_sgsT2_TRta_SD1(i,j,k);
                 denomh_sgsMOT2_TRta_SD1_GA = denomh_sgsMOT2_TRta_SD1_GA  + denomh_sgsMOT2_TRta_SD1(i,j,k);
                 denomh_sgsT2ta_ST_SD1_GA = denomh_sgsT2ta_ST_SD1_GA + denomh_sgsT2ta_ST_SD1(i,j,k);
                 denomh_sgsMOT2ta_ST_SD1_GA = denomh_sgsMOT2ta_ST_SD1_GA  + denomh_sgsMOT2ta_ST_SD1(i,j,k);
                 ...
                 denom_MixL_sgsT2_TRta_SD1_GA = denom_MixL_sgsT2_TRta_SD1_GA + denom_MixL_sgsT2_TRta_SD1(i,j,k);
                 denom_MixL_sgsMOT2_TRta_SD1_GA = denom_MixL_sgsMOT2_TRta_SD1_GA  + denom_MixL_sgsMOT2_TRta_SD1(i,j,k);
                 denom_MixL_sgsT2ta_ST_SD1_GA = denom_MixL_sgsT2ta_ST_SD1_GA + denom_MixL_sgsT2ta_ST_SD1(i,j,k);
                 denom_MixL_sgsMOT2ta_ST_SD1_GA = denom_MixL_sgsMOT2ta_ST_SD1_GA  + denom_MixL_sgsMOT2ta_ST_SD1(i,j,k);
             ...
             elseif (Y_f(j) < -33.3)  %%% -33.3 For channel 
                 ...
                 denomh_sgsT2_TRta_SD2_GA = denomh_sgsT2_TRta_SD2_GA + denomh_sgsT2_TRta_SD2(i,j,k);
                 denomh_sgsMOT2_TRta_SD2_GA = denomh_sgsMOT2_TRta_SD2_GA  + denomh_sgsMOT2_TRta_SD2(i,j,k);
                 denomh_sgsT2ta_ST_SD2_GA = denomh_sgsT2ta_ST_SD2_GA + denomh_sgsT2ta_ST_SD2(i,j,k);
                 denomh_sgsMOT2ta_ST_SD2_GA = denomh_sgsMOT2ta_ST_SD2_GA  + denomh_sgsMOT2ta_ST_SD2(i,j,k);
                 ...
                 denom_MixL_sgsT2_TRta_SD2_GA = denom_MixL_sgsT2_TRta_SD2_GA + denom_MixL_sgsT2_TRta_SD2(i,j,k);
                 denom_MixL_sgsMOT2_TRta_SD2_GA = denom_MixL_sgsMOT2_TRta_SD2_GA  + denom_MixL_sgsMOT2_TRta_SD2(i,j,k);
                 denom_MixL_sgsT2ta_ST_SD2_GA = denom_MixL_sgsT2ta_ST_SD2_GA + denom_MixL_sgsT2ta_ST_SD2(i,j,k);
                 denom_MixL_sgsMOT2ta_ST_SD2_GA = denom_MixL_sgsMOT2ta_ST_SD2_GA  + denom_MixL_sgsMOT2ta_ST_SD2(i,j,k);
             end
        end
    end
end
...
%%%%%% 95% confidence 
z_s=1.96;
KHTH_GhT21_p95 = z_s * KHTH_GhT21_SD *(nfxy);
KHTH_TRtaGhT21_p95 = z_s * KHTH_TRtaGhT21_SD *(nfxy);
KHTH_STGhT21_p95 = z_s * KHTH_STGhT21_SD *(nfxy);
KHTH_GhMOT21_p95 = z_s * KHTH_GhMOT21_SD *(nfxy);
KHTH_TRtaGhMOT21_p95 = z_s * KHTH_TRtaGhMOT21_SD *(nfxy);
KHTH_STGhMOT21_p95 = z_s * KHTH_STGhMOT21_SD *(nfxy);

Lmix_T21_p95 = z_s * Lmix_T21_SD *(nfxy);
Lmix_TRtaT21_p95 = z_s * Lmix_TRtaT21_SD *(nfxy);
Lmix_STT21_p95 = z_s * Lmix_STT21_SD *(nfxy);
Lmix_MOT21_p95 = z_s * Lmix_MOT21_SD *(nfxy);
Lmix_TRtaMOT21_p95 = z_s * Lmix_TRtaMOT21_SD *(nfxy);
Lmix_STMOT21_p95 = z_s * Lmix_STMOT21_SD *(nfxy);

KHTH_GhT22_p95 = z_s * KHTH_GhT22_SD *(nfxy);
KHTH_TRtaGhT22_p95 = z_s * KHTH_TRtaGhT22_SD *(nfxy);
KHTH_STGhT22_p95 = z_s * KHTH_STGhT22_SD *(nfxy);
KHTH_GhMOT22_p95 = z_s * KHTH_GhMOT22_SD *(nfxy);
KHTH_TRtaGhMOT22_p95 = z_s * KHTH_TRtaGhMOT22_SD *(nfxy);
KHTH_STGhMOT22_p95 = z_s * KHTH_STGhMOT22_SD *(nfxy);

Lmix_T22_p95 = z_s * Lmix_T22_SD *(nfxy);
Lmix_TRtaT22_p95 = z_s * Lmix_TRtaT22_SD *(nfxy);
Lmix_STT22_p95 = z_s * Lmix_STT22_SD *(nfxy);
Lmix_MOT22_p95 = z_s * Lmix_MOT22_SD *(nfxy);
Lmix_TRtaMOT22_p95 = z_s * Lmix_TRtaMOT22_SD *(nfxy);
Lmix_STMOT22_p95 = z_s * Lmix_STMOT22_SD *(nfxy);

%%%%%%%%%%%%%%%%%% STANDARD DEVIATION FOR REGRESSION   

SD_VEL_G = ((VEL_sgsta_G ./ AR_fxy) - VEL_sgsta_GA).^2;
SD_VEL_TRG = ((VEL_sgsta_TRG ./ AR_fxy) - VEL_sgsta_TRGA).^2;
SD_VEL_STG = ((VEL_sgsta_STG ./ AR_fxy) - VEL_sgsta_STGA).^2;

%%% NORTH
SD_VEL_G1 = ((VEL_sgsta_G ./ AR_fxy) - VEL_sgsta_GA1).^2;
SD_VEL_TRG1 = ((VEL_sgsta_TRG ./ AR_fxy) - VEL_sgsta_TRGA1).^2;
SD_VEL_STG1 = ((VEL_sgsta_STG ./ AR_fxy) - VEL_sgsta_STGA1).^2;


%%% SOUTH

SD_VEL_G2 = ((VEL_sgsta_G ./ AR_fxy) - VEL_sgsta_GA2).^2;
SD_VEL_TRG2 = ((VEL_sgsta_TRG ./ AR_fxy) - VEL_sgsta_TRGA2).^2;
SD_VEL_STG2 = ((VEL_sgsta_STG ./ AR_fxy) - VEL_sgsta_STGA2).^2;
...
for k=1:zsize
    for i=1:(xsize_f)  %%%
        for j=1:(ysize_f)
                 ...
                 SD_VEL_GA   = SD_VEL_GA + (SD_VEL_G(i,j,k));
                 SD_VEL_TRGA = SD_VEL_TRGA + (SD_VEL_TRG(i,j,k));
                 SD_VEL_STGA = SD_VEL_STGA + (SD_VEL_STG(i,j,k));
            
             if(Y_f(j) >= -33.3)    %%% -33.3 For channel
                 SD_VEL_GA1   = SD_VEL_GA1 + (SD_VEL_G1(i,j,k));
                 SD_VEL_TRGA1 = SD_VEL_TRGA1 + (SD_VEL_TRG1(i,j,k));
                 SD_VEL_STGA1 = SD_VEL_STGA1 + (SD_VEL_STG1(i,j,k));
                 
             elseif (Y_f(j) < -33.3)  %%% -33.3 For channel 
                 SD_VEL_GA2   = SD_VEL_GA2 + (SD_VEL_G2(i,j,k));
                 SD_VEL_TRGA2 = SD_VEL_TRGA2 + (SD_VEL_TRG2(i,j,k));
                 SD_VEL_STGA2 = SD_VEL_STGA2 + (SD_VEL_STG2(i,j,k));
...
SD_VEL_GA = (SD_VEL_GA/sum_glob)^0.5;
SD_VEL_TRGA = (SD_VEL_TRGA/sum_glob)^0.5;
SD_VEL_STGA = (SD_VEL_STGA/sum_glob)^0.5;

SD_VEL_GA1 = (SD_VEL_GA1/sum_glob1)^0.5;
SD_VEL_TRGA1 = (SD_VEL_TRGA1/sum_glob1)^0.5;
SD_VEL_STGA1 = (SD_VEL_STGA1/sum_glob1)^0.5;

SD_VEL_GA2 = (SD_VEL_GA2/sum_glob2)^0.5;
SD_VEL_TRGA2 = (SD_VEL_TRGA2/sum_glob2)^0.5;
SD_VEL_STGA2 = (SD_VEL_STGA2/sum_glob2)^0.5;

%%%%%% 95% confidence 
z_s=1.96;

SD_VEL_GA_p95 = z_s*((SD_VEL_GA)/(sum_glob)^0.5)*(nfxy);
SD_VEL_TRGA_p95 = z_s*((SD_VEL_TRGA)/(sum_glob)^0.5)*(nfxy);
SD_VEL_STGA_p95 = z_s*((SD_VEL_STGA)/(sum_glob)^0.5)*(nfxy);


SD_VEL_GA1_p95 = z_s*((SD_VEL_GA1)/(sum_glob1)^0.5)*(nfxy);
SD_VEL_TRGA1_p95 = z_s*((SD_VEL_TRGA1)/(sum_glob1)^0.5)*(nfxy);
SD_VEL_STGA1_p95 = z_s*((SD_VEL_STGA1)/(sum_glob1)^0.5)*(nfxy);

SD_VEL_GA2_p95 = z_s*((SD_VEL_GA2)/(sum_glob2)^0.5)*(nfxy);
SD_VEL_TRGA2_p95 = z_s*((SD_VEL_TRGA2)/(sum_glob2)^0.5)*(nfxy);
SD_VEL_STGA2_p95 = z_s*((SD_VEL_STGA2)/(sum_glob2)^0.5)*(nfxy);
...
MASK_CORP_FBC = ones(ysize_f,zsize,'single');  %%%%%% MAKS CORUPTION for FBC
for j=1:ysize_f
    if (Y_f(j)<-40.31)
        MASK_CORP_FBC(j,1) = 0;
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
MASK_CORP_CWR = ones(ysize_f,zsize,'single');  %%%%%% MAKS CORUPTION for CWR
for j=1:ysize_f
    if (Y_f(j)<-41.07)
        MASK_CORP_CWR (j,1) = 0;
    end
...
%%% WRITE OUTPUT FILES IN .MAT FORMAT

mkdir FBC  %% CWR : CHANGE THIS FOR CHANNEL WITH BOTTOM TOPOGRAPHY (CWR)
cd FBC     %% CWR
mkdir Filt_41  %%% CHANGE THIS IF YOU USE DIFFERENT FILTER WIDTH nfx, nfy 
cd Filt_41
save('Output1_EDD.mat','vgh_sgsta','ugh_sgsta','nl_grad_vgh_sgsta','nl_grad_ugh_sgsta','GM_vgh_sgsta','GM_ugh_sgsta','KH_vgh_sgsta','KH_ugh_sgsta','RHS_NLX_sgsta','RHS_GMX_sgsta','RHS_NLY_sgsta','RHS_GMY_sgsta','RHS_ACX_sgsta','RHS_ACY_sgsta','X','Y','-v7.3');
save('Output2_MOM.mat','ugug_sgsta','vgvg_sgsta','ugvg_sgsta','nl_grad_ugug_sgsta','nl_grad_vgvg_sgsta','nl_grad_ugvg_sgsta', 'X', 'Y','-v7.3');
save('Output2_MOM_h.mat','ugUg_sgsta','UgUg_sgsta','vgVg_sgsta','VgVg_sgsta','ugVg_sgsta','vgUg_sgsta','UgVg_sgsta','nl_grad_ugUg_sgsta','nl_grad_UgUg_sgsta','nl_grad_vgVg_sgsta','nl_grad_VgVg_sgsta','nl_grad_ugVg_sgsta','nl_grad_vgUg_sgsta','nl_grad_UgVg_sgsta','h_fxy_momta','X','Y','MASK','-v7.3');
save('Output3_GRIDS.mat','DX_fxy','DY_fxy','AR_fxy','AR6_fxy','X','Y','-v7.3');      
cd ..
cd ..
return
