...
function [ugh_SGS,vgh_SGS,h_ave,vg_ave,ug_ave,vg,ugh_fxy,ug_fxyh_fxy,vgh_fxy,vg_fxyh_fxy,ug_sgs,vg_sgs,h_sgs]=SGSflux_cheap_NV(eta,f,gp,dx,dy,kfilt,eps,crops)
...
for k=1:size(h,3)
 ...
 ugh_ave(:,:,k)=centmean_cheap_NV(squeeze(Mask(:,:,k).*ug(:,:,k).*h(:,:,k)).*A,kfilt)./A_ave;
 vgh_ave(:,:,k)=centmean_cheap_NV(squeeze(Mask(:,:,k).*vg(:,:,k).*h(:,:,k)).*A,kfilt)./A_ave;
end
